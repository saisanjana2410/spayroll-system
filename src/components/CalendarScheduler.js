...
const CalendarScheduler = (props) => {
  ...
  const fetchEvents = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/calendar/fetchEvents');
  ...
  if (title && start && end) {
    try {
      const response = await fetch('http://localhost:5000/api/calendar/putEvents', {
        method: 'POST',
        headers: {
...
const handleDelete = async (event) => {
  try {
    const response = await fetch(`http://localhost:5000/api/calendar/deleteEvent/${event._id.toString()}`, {
      method: 'DELETE',
    });

...
const TaskMonitor = (props) => {
  ...
  let handleDelete = async ()=>{
    try {
      const response = await fetch(`http://localhost:5000/api/tasks/deleteTask/${props.id.toString()}`, {
        method: 'DELETE',
        headers: {
  ...
  const handleModalSubmit = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/checkpoints/putCheckpoint', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const fetchCheckpoints = async() =>{
    try {
      const response = await fetch('http://localhost:5000/api/checkpoints/getCheckpoints', {
        method: 'GET',
        headers: {
            'Content-Type' : 'application/json',
            'coll_name' : props.coll_name
